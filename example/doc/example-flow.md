...
```mermaid
flowchart TD
    UI[User Input] -->|Strategy & Policy changes| State[State: GeneratorState]
    ...
    subgraph Validation
        ...
        Gen --> Validator[NormalizedPasswordValidator]
...
- `_updateGenerator()` selects:
  ...
  - Validator: optional `BlocklistPasswordValidator` wrapped inside `NormalizedPasswordValidator` when enabled.
  - Estimator: optional zxcvbn when toggled.
  - Feedback provider: contextual provider (`_ExampleFeedbackProvider`) composing `PasswordFeedbackBuilder` to show localized policy and blocklist warnings.
...
sequenceDiagram
    ...
    participant State as GeneratorState
    participant G as PasswordGenerator
    participant S as Strategy
    participant F as FeedbackProvider

    U->>UI: Tap Generate New Password
    UI->>State: generatePassword()
    State->>State: _updateGenerator + _applyConfig
    State->>G: generatePassword()
    G->>S: generate(config)
    S-->>G: password
    State->>G: estimateFeedback(password)
    G->>F: buildWithContext(context)
    F-->>G: PasswordFeedback
    G-->>State: PasswordFeedback
...
sequenceDiagram
    ...
    UI->>State: generateStrongPassword()
    State->>State: _updateGenerator + _applyConfig
    State->>G: refreshPassword()
